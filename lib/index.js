 ...
 */
module.exports = function chpm(argv, options) {
  return new Promise((resolve, reject) => {
    ...
    if (!options.npmPath || path.extname(options.npmPath) === '.js') {
      ...
      if( npmPath ) execPath = process.execPath
    ...
    if( !execPath ) {
      const optionsFromConfig = require('cosmiconfig')('chpm').searchSync() || {config: []}
      execPath = optionsFromConfig.config.find( pmname => commandExistsSync(pmname) ) || 'npm'
    }
